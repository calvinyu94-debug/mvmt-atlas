...
| `select` | an FMA concept id, a BP3D part id, or an MVMT id (`knee-acl`, `lm-asis`, `ZA-iliotibial-tract`) | Selects and opens the detail panel. An MVMT structure id selects everything the id bridge gives it: its own meshes and the BodyParts3D parts matched to it. |
| `view` | `anterior`, `posterior`, `left`, `right`, `superior` | Camera preset. |
| `patient` | `1` or `0` | Patient view: hides identifiers, counts, the source link and credits; of a fascial line it keeps the highlight and the blurb and hides the station list; of the structure panel it keeps *What it does* and hides the clinical block and the drills. |
...
The viewer posts back to its parent: `{type:'ready', model, parts}` once a manifest has loaded, `{type:'select', id, name, mvmtId}` on every selection change (`null` when cleared; `mvmtId` is the MVMT structure the selection resolves to through the id bridge, or `null`), `{type:'unresolved', id}` when a requested selection is not in the atlas, and `{type:'unresolved', line}` when a requested line is not one of the twelve. The contract lives in [`app/embed.ts`](app/embed.ts).

### The structure panel

Selecting anything with an MVMT counterpart — a BodyParts3D muscle, bone, ligament or fascia the id bridge maps, or one of our own parts — opens the same panel the Z-Anatomy viewer shows in mvmt-program: the structure's name and Latin, its region and system, *What it does*, *Why it matters clinically*, and *Drills that load this* with an Add button on each. The blurbs and the drills come from the parent frame: on `select` the parent answers `{type:'structure', mvmtId, structure, drills, where, hasProgram, inherited}` from its live exercise library; Add posts `{type:'add-exercise', exerciseId, structureId}`, the parent adds the drill to the current phase with the same function its own panels use and answers `{type:'added', exerciseId, structureId, added}`, and the button reads *Added*. Standalone, or with a parent that does not answer, the panel shows the blurbs from a bundled copy (`models/mvmt-structures.json`) and no drills. Patient view keeps *What it does* and hides the clinical block and the drills.

The id bridge is `atlas.layers.mvmt` in `atlas.json`, built by `scripts/build-index.mjs`: every part to the MVMT structures that claim it (the most specific group first, so a click on the sternocostal part of pectoralis major answers Pectoralis Major and offers the head under *Breaks down into*), and every structure to its parts. BodyParts3D parts are matched by name and by the hand tables in `scripts/overrides.mjs` (`CONCEPT_MATCHES` by concept name, `PART_MATCHES` by part id for the few parts whose concept is a parent of others); `verification/bridge/coverage.md` lists which parts resolve to a structure and which do not, and every MVMT structure with no geometry in the atlas with the reason from `ABSENT_STRUCTURES`, which the build requires for each.
...
Validation covers mesh buffers, names and concept membership, that no two carried muscles in one region sit inside each other on more than 2% of their triangles, that every part the overview copies whole is byte-identical to its region chunk, nonoverlapping exploded layouts at desktop and mobile aspect ratios, search and inspection contracts, and tap-versus-drag handling. Browser interaction checks have exercised selection, system controls, search, isolation, rotation, and 390×844, 320×568, and 844×390 layouts. Phone controls stay clear of the exploded inventory, and isolated structures fit the space above or beside the detail panel. Physical-device performance and real multitouch hardware have not been tested.
